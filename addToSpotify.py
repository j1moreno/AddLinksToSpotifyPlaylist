...
def get_spotify_ids_from_file(file_name):
...
def list_all_tracks_in_playlist(sp, playlist):
...
def get_last_track_id(sp, playlist):
...
def main():
    ...
    track_ids_from_file = get_spotify_ids_from_file(args.input)
    ...
    for playlist in playlists["items"]:
        if playlist["name"] == args.playlistName:
            ...
            if playlist["tracks"]["total"] == len(
                    track_ids_from_file) and not args.append:
                ...
                if playlist["tracks"]["total"] == 0 or args.append:
                    ...
                    # if playlist is not empty, get last id currently in the
                    # playlist
                    last_track_id = get_last_track_id(sp, playlist)
                    print("last track id in playlist is", last_track_id)
                    # find this id in the list of id pulled from file, and get
                    # its last index (last because it may be here more than once)
                    all_matches = [
                        i for i, e in enumerate(track_ids_from_file)
                        if e == last_track_id
                    ]
                    if len(all_matches) < 1:
                        # last track in text not found in plist, this is a
                        # different set of tracks; should be appended instead
                        raise Exception(
                            ("parsed links don't match playlist contents. "
                             "use --append option to add new set of tracks"))
                    ...
                    if index_to_start_from == len(track_ids_from_file):
                        ...
                        exit()
                # check for track limit 100; if exceeded,
